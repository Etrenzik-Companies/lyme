...
/**
 * Guard for every route that touches the database.
 *
 * The D1 binding can legitimately be absent — a fresh deployment before the database has
 * been provisioned, or an API token that could not create it. When that happens the site
 * should still serve: readers get an empty list and a clear notice, writers get an honest
 * "not accepting submissions yet" instead of a stack trace.
 *
 * @returns {Response|null} a 503 to return immediately, or null when the DB is ready.
 */
export function requireDb(env, { readOnly = false } = {}) {
  if (env.DB) return null;
  return json(
    {
      error: readOnly
        ? 'This data is not available yet — the site is still being set up.'
        : 'Submissions are not open yet. The database for this site is still being configured.',
      unconfigured: true,
    },
    503,
    { 'cache-control': 'no-store' }
